 ...
 *
 * 모든 공개 페이지의 <head> 상단(구글 태그(gtag.js, #167) 바로 다음)에 GTM
 * 스크립트를, <body> 여는 태그 바로 뒤에 noscript iframe 을 심어야 컨테이너가
 * 정상 동작한다. 위치가 어긋나면 GTM 콘솔이 "설치 확인 안 됨" 으로 표시하므로
 * 위치까지 함께 검증한다.
 *
...
final class GoogleTagManagerTest extends CIUnitTestCase
{
    ...
    public function testHeadStartsWithGtmScript(string $path): void
    {
        ...
        $this->assertMatchesRegularExpression(
            '/gtag\(\'config\', \'G-W1YQS7P9RH\'\);\s*<\/script>\s*<script>\(function\(w,d,s,l,i\).*?GTM-NVTRSRQD.*?<\/script>/s',
            $html,
            'GTM 스크립트가 구글 태그(gtag.js) 스니펫 바로 다음에 없다.'
        );
